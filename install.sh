...
case "$OS" in
    Linux)
        case "$ARCH" in
            ...
            aarch64|arm64)
                ...
                ;;
            i386|i686)
                BINARY_NAME="dops_linux-386-static"
                ;;
            armv5*|armv5tel)
                BINARY_NAME="dops_linux-arm32v5"
                ;;
            armv6*|armv6l)
                BINARY_NAME="dops_linux-arm32v6"
                ;;
            armv7*|armv7l)
                BINARY_NAME="dops_linux-arm32v7"
                ;;
            *)
    ...
    FreeBSD)
        case "$ARCH" in
            amd64|x86_64)
                BINARY_NAME="dops_freebsd-amd64"
                ;;
            arm64|aarch64)
                BINARY_NAME="dops_freebsd-arm64"
                ;;
            *)
    ...
    OpenBSD)
        case "$ARCH" in
            amd64|x86_64)
                BINARY_NAME="dops_openbsd-amd64"
                ;;
            arm64|aarch64)
                BINARY_NAME="dops_openbsd-arm64"
                ;;
            *)
                error "Unsupported OpenBSD architecture: ${ARCH}"
                ;;
        esac
        ;;
    *)
...
if [ -z "$BINARY_NAME" ]; then
    error "Failed to determine binary name for platform: ${OS}/${ARCH}"
fi
